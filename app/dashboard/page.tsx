...
async function ConnectionCards({ userId }: { userId: string }) {
...
function ConnectionCardsSkeleton() {
  return (
    <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {[1, 2, 3].map((i) => (
...
export default async function Dashboard() {
  // Single Clerk call via ensureUser (React.cache-deduped) — no separate auth().
  ...
  return (
    <DashboardShell active="Connections" name={name} email={profile.email}>
      {/* Header */}
      ...
      {/* Cards — streamed in via Suspense so the shell renders instantly */}
      <Suspense fallback={<ConnectionCardsSkeleton />}>
        <ConnectionCards userId={profile.id} />
      </Suspense>
    </DashboardShell>
  );
}
